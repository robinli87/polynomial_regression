...
def backpropagation():
    ...
    def modififyWeight(indices):
        ...
        vw[indices] = gamma * vw[indices] + (1 - gamma) * gradient ** 2
        modifier = learning_rate * gradient * (vw[indices] ** -0.5)
        #print("weight gradient = ", gradient)
        coefficients[indices] -= modifier #* (math.e ** (0.001 / gradient))
